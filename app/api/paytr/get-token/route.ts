...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: order, error: orderError } = await supabase
      .from("orders")
      .select(
        "id, order_number, customer_name, customer_phone, customer_email, delivery_address, total_tl, order_items(product_name_tr, quantity, unit_price_tl)"
      )
      .eq("id", orderId)
      .single();
    ...
    if (!email) {
      return NextResponse.json(
        { error: "Bu sipariş için e-posta adresi kaydedilmemiş." },
        { status: 400 }
      );
    }
